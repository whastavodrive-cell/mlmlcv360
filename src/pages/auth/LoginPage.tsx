...
import { Eye, EyeOff, Mail, Lock, TrendingUp, Users, DollarSign, Sun, Moon, Shield, CircleCheck as CheckCircle, X, Sparkles } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const [stats, setStats] = useState<LoginStats>({ totalUsers: 0, totalPaid: 0, activeRanks: 0 });
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const rankCount = (ranksRes?.data as unknown[])?.length || 0;
        setStats({ totalUsers: userCount, totalPaid: paidAmount, activeRanks: rankCount });
      } catch {
        setStats({ totalUsers: 0, totalPaid: 0, activeRanks: 0 });
      }
    };
    fetchStats();
  }, [backend.database]);
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div className="max-w-md">
            ...
            <p className="text-blue-100/50 text-base leading-relaxed mb-10">
              La plataforma MLM empresarial mas completa. Comisiones automaticas, genealogia en tiempo real y bonos exclusivos.
            </p>
            ...
            <div className="grid grid-cols-3 gap-4 mb-10">
              <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-center">
                ...
                <div className="text-xs text-blue-300/50">Afiliados activos</div>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-center">
                ...
                <div className="text-xs text-blue-300/50">En comisiones</div>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-center">
                ...
                <div className="text-xs text-blue-300/50">Rangos activos</div>
              </div>
            </div>

            <div className="space-y-3">
              {[
                { text: 'Comisiones calculadas automaticamente' },
                { text: 'Arbol genealogico en tiempo real' },
                { text: 'Sistema de rangos con bonos' },
              ].map(item => (
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5">
            <p className="text-white/60 text-sm italic mb-3">
              "Alcance el rango Oro en 6 meses. El sistema es simple y efectivo."
            </p>
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-xs font-bold text-white">M</div>
              <div>
                <div className="text-sm font-medium text-white">Maria Elena C.</div>
                <div className="text-xs text-blue-300/50">Rango Oro, Arequipa</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/registro" className="text-sm font-medium text-primary hover:text-primary/80 transition-colors">
              Crear cuenta
            </Link>
          </div>
        </div>
          ...
          <div className="w-full max-w-sm">
            ...
            {googleOAuthEnabled && (
              <>
                <button onClick={handleGoogle} disabled={googleLoading}
                  className="w-full border border-border hover:bg-muted py-2.5 rounded-xl transition-colors flex items-center justify-center gap-3 text-sm font-medium text-foreground disabled:opacity-50 mb-4">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" {...register('email')} placeholder="tu@correo.com"
                    className={cn('w-full pl-9 pr-4 py-2.5 bg-muted border rounded-xl text-foreground text-sm placeholder:text-muted-foreground outline-none transition-colors',
              ...
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="remember" className="text-sm text-muted-foreground cursor-pointer">Mantener sesion abierta</label>
              </div>
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <><Sparkles className="w-4 h-4" /> Acceder ahora</>}
              </button>
            </form>
          ...
          <div className="bg-card border border-border rounded-2xl w-full max-w-sm shadow-2xl p-6">
            ...
            {forgotSent ? (
              <div className="text-center py-4">
                ...
                <p className="text-sm text-foreground mb-4">Te enviamos un correo a <strong>{forgotEmail}</strong> con instrucciones.</p>
